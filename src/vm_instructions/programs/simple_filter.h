#pragma once

std::vector<vm_inst> make_simple_filter_program()
{
    const auto ETH_P_IP = 0x0008;
    const auto ETH_P_IPV6 = 0xDD86;
    const unsigned int IPPROTO_ICMP = 1;
    const unsigned int IPPROTO_ICMPV6 = 58;

    return {
        // struct xdp_md *ctx 

        //  r1 = ctx->h_proto
        // h_proto field is at offset 12 in Ethernet header 
        // in original program ethhdr (Ethernet header) is defined as ctx->data which is at offset 0 so should just be 12 offset 
        {OP_READ_CTX, 1, 0, 2, 12},

        // r2 = 0x0008 
        // (ETH_P_IP read as little-endian, so already converted, no need for bpf_htons)
        {OP_LOAD, 2, 0, ETH_P_IP, 0},

        // r3 = 0xDD86
        // same as before ETH_P_IPV6 = 0x86DD which is 0xDD86 as little-endian
        {OP_LOAD, 3, 0, ETH_P_IPV6, 0},

        // So now we have eth-h_proto in register 1 and the two cases we want ot cheack 
        // case 1: bpf_htons(ETH_P_IP) = r2 and bpf_htons(ETH_P_IPV6) = r3

        //============================ case 1 ============================
        // case 1 (not ipv4): bpf_htons(ETH_P_IP) if r1 != r2 -> PASS
        {OP_JNEQ, 1, 2, 8, 0},

        // r4 = ctx->protocol
        // in code it is:
        // ip = data+sizeof(strct ethhdr) ethhdr is 14 bytes 
        // ip is the start of the IPv4 header and protocol is at offset 9 
        // therfore 14 + 9 = 23
        // Read 1 byte from ctx->data + 23 (IPv4 protocol field)
        {OP_READ_CTX, 4, 0, 1, 23},

        // r5 = 1 (IPPROTO_ICMP)
        {OP_LOAD, 5, 0, IPPROTO_ICMP, 0},

        // if r1 != r2 -> PASS
        {OP_JNEQ, 4, 5, 3, 0},

        // ---- DROP ----
        {OP_LOAD, 0, 0, 1, 0},   // return XDP_DROP
        {OP_RINGBUF, 0, 0, 0, 0},
        {OP_EXIT, 0, 0, 0, 0},

        //============================ case 2 ============================
        // case 2 (not ipv6): bpf_htons(ETH_P_IPV6) = r3 != r1 -> pass
        {OP_JNEQ, 1, 3, 8, 0},

        // r4 = ip6->nexthdr
        {OP_READ_CTX, 4, 0, 1, 20},

        // r5 = 1 (IPPROTO_ICMP)
        {OP_LOAD, 5, 0, IPPROTO_ICMPV6, 0},

        // if r1 != r2 -> PASS
        {OP_JNEQ, 4, 5, 3, 0},

        // ---- DROP ----
        {OP_LOAD, 0, 0, 1, 0},   // return XDP_DROP
        {OP_RINGBUF, 0, 0, 0, 0},
        {OP_EXIT, 0, 0, 0, 0},
    
        // ---- PASS ----
        {OP_LOAD, 0, 0, 2, 0},   // return XDP_PASS
        {OP_EXIT, 0, 0, 0, 0}
    };
}