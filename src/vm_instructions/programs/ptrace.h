...
std::vector<vm_inst> make_ptrace_program(pid_t protected_pid)
{
    return {
        ...
        vm_inst{OP_JNEQ, 1, 2, 3, 0},    // 04) if r1(protected pid) != r2(target pid): jump to exit (pc +3)
        vm_inst{OP_JEQ, 1, 0, 2, 0},    // 04) if r1(protected pid) == r0(caller pid): jump to exit (pc +2)
