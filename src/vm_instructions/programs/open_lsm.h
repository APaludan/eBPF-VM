...
std::vector<vm_inst> make_lsm_open_program(pid_t protected_pid)
{
    ...
    return {
        ...
        vm_inst{OP_JNEQ, 1, 0, 3, 0},             // 04) ,3
        vm_inst{OP_LOAD, 0, 0, 0, 0},         // 05) 
        ...
        vm_inst{OP_JEQ, 5, 6, 3, 0},                      // 12) ,11
        vm_inst{OP_LOAD, 0, 0, 0, 0},                 // 13) 
        ...
        vm_inst{OP_JNEQ, 0, 8, 2, 0},                   // 19) exit if read failed, means it is probably not procfs anyway idk
